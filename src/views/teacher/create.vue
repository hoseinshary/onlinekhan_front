<template>
  <base-modal-create
    ...
    :show="teacherStore.openModal.create"
    size="lg"
    @confirm="teacherStore.submitCreate"
    @reset="teacherStore.resetCreate"
    @open="open"
    @close="teacherStore.OPEN_MODAL_CREATE(false)"
  >
...
import { teacherValidations } from "src/validations/teacher/teacherCreateValidation";
import { UserType } from "../../utilities/enumeration";

@Component({
  validations: teacherValidations
})
export default class TeacherCreateVue extends Vue {
  $v: any;
  ...
  //#region ### hooks ###
  created() {
    this.teacherStore.SET_CREATE_VUE(this);
  }
  //#endregion
}
</script>
